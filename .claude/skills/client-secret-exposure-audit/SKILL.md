...
- Do **not** use this to attack third-party sites. See _Security & Safety Notes_.
...
A key being "test/demo" does not make it safe if the _pattern_ would ship a live
...
- **Problem:** Every probed path returns HTTP 200, hiding real files.
  **Solution:** SPA catch-all rewrite. Baseline the fallback body size and only
  treat paths with a _different_ size as real files.
