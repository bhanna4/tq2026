...
- `playwright.config.ts` — central Playwright configuration. Tests run against three browser projects (chromium, firefox, webkit), each `testMatch: '**/ui/**'`, plus an `api` project (no browser device, `baseURL: 'https://gorest.co.in'`, `testMatch: '**/api/**'`) for REST API tests. `baseURL` and `webServer` for the UI projects are commented out, so UI tests currently navigate to absolute URLs directly rather than a local app. There is also a `setup` project (`testMatch: '**/setup/**'`) that the `chromium`/`firefox`/`webkit` projects declare as a `dependencies` entry; it logs into BearStore once and its `storageState` (from `fixtures/auth-storage.ts`) is reused by those projects so UI tests start already authenticated.
